function [time, pos_jointspace] = pick_and_place_motion(positions,joint_max_vel,joint_max_acc,task_max_vel,task_max_acc,task_nom_vel,task_nom_acc)

P_home = [0;0;356.521686];

P_pick = get_target_position(positions(1:2));
% approach object
...
pos_jointspace = [pos_jointspace1 pos_jointspace2 pos_jointspace3 pos_jointspace4];
time = [time1 time2+t1 time3+t2 time4+t3];

for i = 1:2:length(positions)-2
    P_drop = get_target_position(positions(i+2:i+3));
    P_pick = get_target_position(positions(i:i+1));
    ...
    t7 = t6 + time7(end);
    t8 = t7 + time8(end);
    pos_jointspace = [pos_jointspace pos_jointspace5 pos_jointspace6 pos_jointspace7 pos_jointspace8];
    time = [time time5+t4 time6+t5 time7+t6 time8+t7];
...
pos_jointspace = [pos_jointspace pos_jointspace9];
time = [time time9+time(end)];
end